...
(function(){
  ...
  /* =========================================================
     V19 BUSINESS & EXPERIENCE OS — active owner tools
  ========================================================= */
  function bizHtmlList(rows, renderer, empty='Belum ada data.'){
    const arr=safeArr(rows);return arr.length?arr.map(renderer).join(''):('<div class="muted">'+esc(empty)+'</div>');
  }
  async function loadBusinessComp(){try{const [rules,entries]=await Promise.all([api('/api/admin/owner/business/compensation/rules'),api('/api/admin/owner/business/compensation/entries')]);const el=$('bizCompList');if(el)el.innerHTML=bizHtmlList(rules,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.rule_key)}</h3><p>${esc(r.contributor_type)} · ${esc(r.content_type)} · ${esc(r.basis)} · ${Number(r.share_rate||0)*100}%</p></div><span class="tag">${r.active?'ACTIVE':'OFF'}</span></div></div>`,'Belum ada aturan honor.');const e=$('bizCompLoad');if(e)e.dataset.count=entries.length;}catch(err){if($('bizCompList'))$('bizCompList').innerHTML='<div class="muted">'+esc(err.message)+'</div>';}}
  async function saveBusinessComp(){try{await api('/api/admin/owner/business/compensation/rules',{method:'POST',body:JSON.stringify({rule_key:$('bizCompRuleKey').value,contributor_type:$('bizCompType').value,basis:$('bizCompBasis').value,fixed_amount:Number($('bizCompFixed').value||0),share_rate:Number($('bizCompShare').value||0)/100,content_type:$('bizCompContent').value})});toast('Aturan pembagian uang disimpan.');loadBusinessComp();}catch(e){toast(e.message,true)}}
  async function loadBusinessPricing(){try{const rows=await api('/api/admin/owner/business/pricing/rules');const el=$('bizPriceList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.placement)}</h3><p>${esc(r.channel)} · base ${money(r.base_rate)} · suggested ${money(r.suggested_rate)}</p></div><span class="tag">${esc(r.status)}</span></div></div>`,'Belum ada rate card.');}catch(e){if($('bizPriceList'))$('bizPriceList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  function businessPricingPayload(){return {pricing_key:'PRICE-'+Date.now(),placement:$('bizPricePlacement').value,base_rate:Number($('bizPriceBase').value||0),audience_multiplier:Number($('bizPriceAudience').value||1),demand_multiplier:Number($('bizPriceDemand').value||1),fill_multiplier:Number($('bizPriceFill').value||1),seasonal_multiplier:1,premium_multiplier:Number($('bizPricePremium').value||1),auto_reprice:true,status:'suggested'}}
  async function simulateBusinessPricing(){try{const d=await api('/api/admin/owner/business/pricing/simulate',{method:'POST',body:JSON.stringify(businessPricingPayload())});$('bizPriceResult').textContent='Suggested rate: '+money(d.suggested_rate)+' · formula '+d.formula;}catch(e){$('bizPriceResult').textContent=e.message;}}
  async function saveBusinessPricing(){try{const d=await api('/api/admin/owner/business/pricing/rules',{method:'POST',body:JSON.stringify(businessPricingPayload())});$('bizPriceResult').textContent='Rate card tersimpan · suggested '+money(d.suggested_rate)+' · harga publik tetap perlu approval.';loadBusinessPricing();}catch(e){toast(e.message,true)}}
  async function repriceBusiness(){try{const d=await api('/api/admin/owner/business/pricing/reprice',{method:'POST'});toast('Reprice selesai: '+nf(d.updated)+' rate card.');loadBusinessPricing();}catch(e){toast(e.message,true)}}
  async function loadBusinessPages(){try{const rows=await api('/api/admin/owner/business/site/pages');const el=$('bizPageList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.title)}</h3><p>${esc(r.slug)} · v${esc(r.version)} · ${esc(r.status)}</p></div><button class="secondary" onclick="window.bizUsePage(${JSON.stringify(r).replace(/</g,'\\u003c')})">Edit</button></div></div>`,'Belum ada halaman configuration.');}catch(e){if($('bizPageList'))$('bizPageList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  window.bizUsePage=(r)=>{if(!r)return;['page_key','title','slug','content_html','status'].forEach(k=>{const id={page_key:'bizPageKey',title:'bizPageTitle',slug:'bizPageSlug',content_html:'bizPageHtml',status:'bizPageStatus'}[k];if($(id))$(id).value=r[k]??'';});};
  async function saveBusinessPage(){try{await api('/api/admin/owner/business/site/pages',{method:'POST',body:JSON.stringify({page_key:$('bizPageKey').value,title:$('bizPageTitle').value,slug:$('bizPageSlug').value,content_html:$('bizPageHtml').value,status:$('bizPageStatus').value})});toast('Halaman tersimpan.');loadBusinessPages();}catch(e){toast(e.message,true)}}
  async function loadBusinessNav(){try{const rows=await api('/api/admin/owner/business/site/navigation');const el=$('bizNavList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.label)}</h3><p>${esc(r.nav_key)} · ${esc(r.item_type)} · order ${r.sort_order}</p></div><span class="tag">${r.visible?'VISIBLE':'HIDDEN'}</span></div></div>`,'Belum ada menu configuration.');}catch(e){if($('bizNavList'))$('bizNavList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  async function saveBusinessNav(){try{await api('/api/admin/owner/business/site/navigation',{method:'POST',body:JSON.stringify({nav_key:$('bizNavKey').value,label:$('bizNavLabel').value,href:$('bizNavHref').value,item_type:$('bizNavType').value,sort_order:Number($('bizNavOrder').value||0),visible:true})});toast('Menu tersimpan.');loadBusinessNav();}catch(e){toast(e.message,true)}}
  async function loadBusinessCampaigns(){try{const rows=await api('/api/admin/owner/business/campaigns');const el=$('bizCampList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.title)}</h3><p>${esc(r.campaign_type)} · ${esc(r.status)} · ${esc(r.campaign_key)}</p></div></div></div>`,'Belum ada campaign.');}catch(e){if($('bizCampList'))$('bizCampList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  async function saveBusinessCampaign(){try{await api('/api/admin/owner/business/campaigns',{method:'POST',body:JSON.stringify({campaign_key:$('bizCampKey').value,campaign_type:$('bizCampType').value,title:$('bizCampTitle').value,description:$('bizCampDesc').value,status:'draft'})});toast('Campaign interaktif tersimpan.');loadBusinessCampaigns();}catch(e){toast(e.message,true)}}
  async function loadBusinessCalendar(){try{const rows=await api('/api/admin/owner/business/calendar');const el=$('bizCalList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.title)}</h3><p>${esc(r.event_date)} · ${esc(r.status)} · auto-draft ${r.auto_generate_draft?'ON':'OFF'} · auto-publish ${r.auto_publish?'ON':'OFF'}</p></div></div></div>`,'Belum ada kalender event.');}catch(e){if($('bizCalList'))$('bizCalList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  async function saveBusinessCalendar(){try{await api('/api/admin/owner/business/calendar',{method:'POST',body:JSON.stringify({calendar_key:'CAL-'+($('bizCalDate').value||Date.now()),event_date:$('bizCalDate').value,title:$('bizCalTitle').value,content_angle:$('bizCalAngle').value,auto_generate_draft:$('bizCalDraft').value==='true',auto_publish:$('bizCalPublish').value==='true',status:'planned'})});toast('Kalender konten tersimpan.');loadBusinessCalendar();}catch(e){toast(e.message,true)}}
  async function loadBusinessContacts(){try{const rows=await api('/api/admin/owner/business/contacts');const el=$('bizContactList');if(el)el.innerHTML=rows.length?`<table class="tbl"><thead><tr><th>Nama</th><th>Email</th><th>Telepon</th><th>Consent</th><th>Sumber</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.display_name||'—')}</td><td>${esc(r.email||'—')}</td><td>${esc(r.phone||'—')}</td><td>${esc(r.consent_status)}</td><td>${esc(r.source)}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">Belum ada contact directory.</div>';}catch(e){if($('bizContactList'))$('bizContactList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  async function saveBusinessContact(){try{await api('/api/admin/owner/business/contacts',{method:'POST',body:JSON.stringify({contact_key:'CONTACT-'+Date.now(),display_name:$('bizContactName').value,email:$('bizContactEmail').value,phone:$('bizContactPhone').value,consent_status:$('bizContactConsent').value,purpose:$('bizContactPurpose').value})});toast('Kontak tersimpan dengan audit/consent.');loadBusinessContacts();}catch(e){toast(e.message,true)}}
  async function loadBusinessWidgets(){try{const rows=await api('/api/admin/owner/business/widgets');const el=$('bizWidgetList');if(el)el.innerHTML=bizHtmlList(rows,r=>`<div class="item"><div class="item-row"><div><h3>${esc(r.title)}</h3><p>${esc(r.widget_type)} · ${esc(r.provider||'internal')} · ${r.active?'active':'off'}</p></div><span class="tag">${esc(r.widget_key)}</span></div></div>`,'Belum ada widget.');}catch(e){if($('bizWidgetList'))$('bizWidgetList').innerHTML='<div class="muted">'+esc(e.message)+'</div>';}}
  async function loadBusinessWeather(){try{const lat=Number($('bizLat').value),lon=Number($('bizLon').value);const d=await api('/api/admin/owner/business/weather?lat='+encodeURIComponent(lat)+'&lon='+encodeURIComponent(lon));const c=d.current||{};$('bizWeatherResult').textContent=`${$('bizLocation').value}: ${c.temperature_2m??'—'}° · humidity ${c.relative_humidity_2m??'—'}% · wind ${c.wind_speed_10m??'—'} km/h · updated ${c.time||'—'}`;}catch(e){$('bizWeatherResult').textContent=e.message;}}
  async function loadBusiness(){
    const date=$('bizCalDate');if(date&&!date.value)date.value=new Date().toISOString().slice(0,10);
    await Promise.allSettled([loadBusinessComp(),loadBusinessPricing(),loadBusinessPages(),loadBusinessNav(),loadBusinessCampaigns(),loadBusinessCalendar(),loadBusinessContacts(),loadBusinessWidgets()]);
  }
  document.addEventListener('DOMContentLoaded',()=>{
    $('bizCompSave')?.addEventListener('click',saveBusinessComp);$('bizCompLoad')?.addEventListener('click',loadBusinessComp);
    $('bizPriceSim')?.addEventListener('click',simulateBusinessPricing);$('bizPriceSave')?.addEventListener('click',saveBusinessPricing);$('bizPriceReprice')?.addEventListener('click',repriceBusiness);
    $('bizPageSave')?.addEventListener('click',saveBusinessPage);$('bizPageRefresh')?.addEventListener('click',loadBusinessPages);$('bizNavSave')?.addEventListener('click',saveBusinessNav);$('bizNavRefresh')?.addEventListener('click',loadBusinessNav);
    $('bizCampSave')?.addEventListener('click',saveBusinessCampaign);$('bizCampRefresh')?.addEventListener('click',loadBusinessCampaigns);
    $('bizCalSave')?.addEventListener('click',saveBusinessCalendar);$('bizCalRefresh')?.addEventListener('click',loadBusinessCalendar);
    $('bizContactSave')?.addEventListener('click',saveBusinessContact);$('bizContactRefresh')?.addEventListener('click',loadBusinessContacts);
    $('bizWeatherLoad')?.addEventListener('click',loadBusinessWeather);$('bizWidgetRefresh')?.addEventListener('click',loadBusinessWidgets);
    document.querySelector('#nav')?.addEventListener('click',e=>{const b=e.target.closest('button[data-view]');if(b?.dataset.view==='business')loadBusiness();});
  });

  // Stable hooks consumed by the unified V19 workspace router.
  window.MudaBusiness={load:loadBusiness};
  window.MudaRegions={load:loadRegions};
  window.MudaStudio={load:loadStudioProjects};
