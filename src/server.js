...
/*
   V19 BUSINESS & EXPERIENCE OS — migration 035 runtime
   Owner-facing operational tools: creator compensation, dynamic ad pricing,
   website/page/navigation control, interactive campaigns, editorial calendar,
   audience contact directory, weather/map widgets.
*/

function v19MaskPhone(phone){
  const p=String(phone||'').trim();
  if(!p) return null;
  if(p.length<=4) return '••••';
  return p.slice(0,Math.min(3,p.length-4))+'••••'+p.slice(-2);
}

function v19PriceFormula(input){
  const base=Math.max(0,Number(input.base_rate||0));
  const factors=[
    Math.max(0.1,Number(input.audience_multiplier||1)),
    Math.max(0.1,Number(input.demand_multiplier||1)),
    Math.max(0.1,Number(input.fill_multiplier||1)),
    Math.max(0.1,Number(input.seasonal_multiplier||1)),
    Math.max(0.1,Number(input.premium_multiplier||1))
  ];
  return Math.round(base*factors.reduce((a,b)=>a*b,1));
}

app.get('/api/admin/owner/business/compensation/rules', admin, async (_req,res)=>{
  try{const {data,error}=await adminClient.from('owner_compensation_rules').select('*').order('active',{ascending:false}).order('effective_from',{ascending:false});if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}
});
app.post('/api/admin/owner/business/compensation/rules', admin, ownerWrite, async (req,res)=>{
  try{
    const b=req.body||{};
    const row={rule_key:cleanText(b.rule_key,100)||`COMP-${Date.now()}`,contributor_type:cleanText(b.contributor_type,60)||'writer',content_type:cleanText(b.content_type,30)||'article',scope_type:cleanText(b.scope_type,40)||'global',scope_key:cleanText(b.scope_key,120)||null,basis:['fixed','revenue_share','hybrid'].includes(b.basis)?b.basis:'hybrid',fixed_amount:Math.max(0,Number(b.fixed_amount||0)),share_rate:Math.max(0,Math.min(1,Number(b.share_rate||0))),currency:cleanText(b.currency,8).toUpperCase()||'IDR',min_amount:b.min_amount==null?null:Math.max(0,Number(b.min_amount)),max_amount:b.max_amount==null?null:Math.max(0,Number(b.max_amount)),active:b.active!==false,effective_from:b.effective_from||new Date().toISOString(),effective_to:b.effective_to||null,metadata:b.metadata&&typeof b.metadata==='object'?b.metadata:{}};
    const {data,error}=await adminClient.from('owner_compensation_rules').upsert(row,{onConflict:'rule_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'compensation_rule_upserted','owner_compensation_rule',data.id,row);res.status(201).json(data);
  }catch(error){res.status(400).json({error:error.message});}
});
app.get('/api/admin/owner/business/compensation/entries', admin, async (req,res)=>{
  try{let q=adminClient.from('owner_compensation_entries').select('*').order('created_at',{ascending:false}).limit(300);if(req.query.contributor_key)q=q.eq('contributor_key',String(req.query.contributor_key));if(req.query.status)q=q.eq('status',String(req.query.status));const {data,error}=await q;if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}
});
app.post('/api/admin/owner/business/compensation/calculate', admin, ownerWrite, async (req,res)=>{
  try{
    const b=req.body||{};const contributorKey=cleanText(b.contributor_key,160);if(!contributorKey)return res.status(400).json({error:'contributor_key wajib'});let rule=null;
    if(b.rule_key){const r=await adminClient.from('owner_compensation_rules').select('*').eq('rule_key',String(b.rule_key)).maybeSingle();if(r.error)throw r.error;rule=r.data;}
    if(!rule){const r=await adminClient.from('owner_compensation_rules').select('*').eq('active',true).eq('content_type',cleanText(b.content_type,30)||'article').order('effective_from',{ascending:false}).limit(1).maybeSingle();if(r.error)throw r.error;rule=r.data;}
    if(!rule)return res.status(404).json({error:'Aturan kompensasi aktif belum tersedia'});
    const revenue=Math.max(0,Number(b.revenue_base||0));const fixed=rule.basis==='revenue_share'?0:Number(rule.fixed_amount||0);const share=rule.basis==='fixed'?0:revenue*Number(rule.share_rate||0);let amount=fixed+share;if(rule.min_amount!=null)amount=Math.max(amount,Number(rule.min_amount));if(rule.max_amount!=null)amount=Math.min(amount,Number(rule.max_amount));amount=Math.round(amount);
    res.json({rule,contributor_key:contributorKey,revenue_base:revenue,fixed_amount:fixed,share_amount:Math.round(share),calculated_amount:amount,currency:rule.currency});
  }catch(error){res.status(400).json({error:error.message});}
});
app.post('/api/admin/owner/business/compensation/entries', admin, ownerWrite, async (req,res)=>{
  try{const b=req.body||{};const calculation=b.calculation&&typeof b.calculation==='object'?b.calculation:{};const row={contributor_key:cleanText(b.contributor_key,160)||'unknown',contributor_name:cleanText(b.contributor_name,200)||null,content_type:cleanText(b.content_type,30)||'article',content_id:cleanText(b.content_id,120)||null,period_start:b.period_start||null,period_end:b.period_end||null,revenue_base:Math.max(0,Number(b.revenue_base||0)),fixed_amount:Math.max(0,Number(b.fixed_amount||calculation.fixed_amount||0)),share_rate:Math.max(0,Math.min(1,Number(b.share_rate||calculation.share_rate||0))),calculated_amount:Math.max(0,Number(b.calculated_amount||calculation.calculated_amount||0)),currency:cleanText(b.currency,8).toUpperCase()||'IDR',status:['proposed','approved','scheduled','paid','void'].includes(b.status)?b.status:'proposed',rule_key:cleanText(b.rule_key,100)||null,calculation};const {data,error}=await adminClient.from('owner_compensation_entries').insert(row).select().single();if(error)throw error;await auditOwnerAction(req,'compensation_entry_created','owner_compensation_entry',data.id,{amount:data.calculated_amount,contributor_key:data.contributor_key});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}
});

app.get('/api/admin/owner/business/pricing/rules', admin, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_ad_pricing_rules').select('*').order('placement');if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/pricing/rules', admin, ownerWrite, async (req,res)=>{
  try{const b=req.body||{};const factors={audience_multiplier:Number(b.audience_multiplier||1),demand_multiplier:Number(b.demand_multiplier||1),fill_multiplier:Number(b.fill_multiplier||1),seasonal_multiplier:Number(b.seasonal_multiplier||1),premium_multiplier:Number(b.premium_multiplier||1)};const suggested=v19PriceFormula({base_rate:b.base_rate,...factors});const row={pricing_key:cleanText(b.pricing_key,100)||`PRICE-${Date.now()}`,placement:cleanText(b.placement,160)||'web',channel:cleanText(b.channel,80)||'web',region:cleanText(b.region,120)||null,base_rate:Math.max(0,Number(b.base_rate||0)),...factors,suggested_rate:suggested,approved_rate:Math.max(0,Number(b.approved_rate||0)),currency:cleanText(b.currency,8).toUpperCase()||'IDR',auto_reprice:b.auto_reprice===true,status:['draft','suggested','approved','active','archived'].includes(b.status)?b.status:(b.auto_reprice?'suggested':'draft'),effective_from:b.effective_from||new Date().toISOString(),effective_to:b.effective_to||null,formula_version:'v1',metadata:b.metadata&&typeof b.metadata==='object'?b.metadata:{}};const {data,error}=await adminClient.from('owner_ad_pricing_rules').upsert(row,{onConflict:'pricing_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'ad_pricing_rule_upserted','owner_ad_pricing_rule',data.id,{suggested_rate:suggested,approved_rate:data.approved_rate,auto_reprice:data.auto_reprice});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}
});
app.post('/api/admin/owner/business/pricing/simulate', admin, async (req,res)=>{try{const b=req.body||{};res.json({suggested_rate:v19PriceFormula(b),currency:cleanText(b.currency,8).toUpperCase()||'IDR',formula:'base × audience × demand × fill × seasonal × premium'});}catch(error){res.status(400).json({error:error.message});}});
app.post('/api/admin/owner/business/pricing/reprice', admin, ownerWrite, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_ad_pricing_rules').select('*').eq('auto_reprice',true).in('status',['suggested','approved','active']);if(error)throw error;const rows=data||[];let updated=0;for(const r of rows){const suggested=v19PriceFormula(r);const u=await adminClient.from('owner_ad_pricing_rules').update({suggested_rate:suggested,updated_at:new Date().toISOString()}).eq('id',r.id);if(u.error)throw u.error;updated++;}res.json({updated});}catch(error){res.status(400).json({error:error.message});}});

app.get('/api/admin/owner/business/site/pages', admin, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_site_pages').select('*').order('updated_at',{ascending:false});if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/site/pages', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const row={page_key:cleanText(b.page_key,100)||`page-${Date.now()}`,title:cleanText(b.title,240)||'Halaman',slug:cleanText(b.slug,240)||`/page-${Date.now()}.html`,content_html:typeof b.content_html==='string'?b.content_html:'',content_json:b.content_json&&typeof b.content_json==='object'?b.content_json:{},seo_title:cleanText(b.seo_title,240)||null,seo_description:cleanText(b.seo_description,500)||null,status:['draft','published','archived'].includes(b.status)?b.status:'draft',version:Math.max(1,Number(b.version||1)),published_at:b.status==='published'?new Date().toISOString():null,updated_by:req.user.id};const {data,error}=await adminClient.from('owner_site_pages').upsert(row,{onConflict:'page_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'site_page_upserted','owner_site_page',data.id,{page_key:data.page_key,status:data.status,version:data.version});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}});
app.patch('/api/admin/owner/business/site/pages/:id', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const patch={updated_by:req.user.id,updated_at:new Date().toISOString()};for(const k of ['title','slug','content_html','seo_title','seo_description','content_json','status'])if(b[k]!==undefined)patch[k]=b[k];if(b.version!==undefined)patch.version=Math.max(1,Number(b.version));if(patch.status==='published')patch.published_at=new Date().toISOString();const {data,error}=await adminClient.from('owner_site_pages').update(patch).eq('id',req.params.id).select().single();if(error)throw error;await auditOwnerAction(req,'site_page_updated','owner_site_page',data.id,{fields:Object.keys(patch)});res.json(data);}catch(error){res.status(400).json({error:error.message});}});
app.get('/api/admin/owner/business/site/navigation', admin, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_site_navigation').select('*').order('location').order('sort_order');if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/site/navigation', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const row={nav_key:cleanText(b.nav_key,100)||`nav-${Date.now()}`,location:cleanText(b.location,40)||'main',label:cleanText(b.label,120)||'Menu',href:cleanText(b.href,500)||null,item_type:['page','article','video','external','quiz','challenge','custom'].includes(b.item_type)?b.item_type:'page',parent_key:cleanText(b.parent_key,100)||null,sort_order:Number(b.sort_order)||0,visible:b.visible!==false,start_at:b.start_at||null,end_at:b.end_at||null,metadata:b.metadata&&typeof b.metadata==='object'?b.metadata:{},updated_by:req.user.id};const {data,error}=await adminClient.from('owner_site_navigation').upsert(row,{onConflict:'nav_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'site_navigation_upserted','owner_site_navigation',data.id,{nav_key:data.nav_key});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}});
app.patch('/api/admin/owner/business/site/navigation/:id', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const patch={updated_by:req.user.id,updated_at:new Date().toISOString()};for(const k of ['label','href','item_type','parent_key','sort_order','visible','start_at','end_at','metadata','location'])if(b[k]!==undefined)patch[k]=b[k];const {data,error}=await adminClient.from('owner_site_navigation').update(patch).eq('id',req.params.id).select().single();if(error)throw error;await auditOwnerAction(req,'site_navigation_updated','owner_site_navigation',data.id,{fields:Object.keys(patch)});res.json(data);}catch(error){res.status(400).json({error:error.message});}});

app.get('/api/admin/owner/business/campaigns', admin, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_interactive_campaigns').select('*').order('starts_at',{ascending:false,nullsLast:true});if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/campaigns', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const row={campaign_key:cleanText(b.campaign_key,120)||`CAMP-${Date.now()}`,campaign_type:['quiz','challenge','poll','survey','contest'].includes(b.campaign_type)?b.campaign_type:'quiz',title:cleanText(b.title,240)||'Campaign interaktif',description:cleanText(b.description,2000)||null,intro:cleanText(b.intro,1000)||null,config:b.config&&typeof b.config==='object'?b.config:{},reward_config:b.reward_config&&typeof b.reward_config==='object'?b.reward_config:{},status:['draft','scheduled','active','paused','completed','archived'].includes(b.status)?b.status:'draft',starts_at:b.starts_at||null,ends_at:b.ends_at||null,results_visibility:['instant','after_submit','owner_only'].includes(b.results_visibility)?b.results_visibility:'after_submit',created_by:req.user.id};const {data,error}=await adminClient.from('owner_interactive_campaigns').upsert(row,{onConflict:'campaign_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'interactive_upserted','interactive_campaign',data.id,{campaign_type:data.campaign_type,status:data.status});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}});

app.get('/api/admin/owner/business/calendar', admin, async (req,res)=>{try{let q=adminClient.from('owner_editorial_calendar').select('*').order('event_date',{ascending:true}).limit(300);if(req.query.from)q=q.gte('event_date',String(req.query.from));if(req.query.to)q=q.lte('event_date',String(req.query.to));const {data,error}=await q;if(error)throw error;res.json(data||[]);}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/calendar', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};if(!b.event_date)return res.status(400).json({error:'event_date wajib'});const row={calendar_key:cleanText(b.calendar_key,120)||`CAL-${Date.now()}`,event_date:b.event_date,title:cleanText(b.title,240)||'Hari penting',event_type:cleanText(b.event_type,80)||'special_day',country:cleanText(b.country,4)||'ID',region:cleanText(b.region,120)||null,content_angle:cleanText(b.content_angle,3000)||null,suggested_formats:Array.isArray(b.suggested_formats)?b.suggested_formats:[],brand_voice:cleanText(b.brand_voice,500)||null,auto_generate_draft:b.auto_generate_draft!==false,auto_publish:b.auto_publish===true,status:['planned','drafted','scheduled','posted','skipped'].includes(b.status)?b.status:'planned',content_id:cleanText(b.content_id,120)||null,notes:cleanText(b.notes,3000)||null};const {data,error}=await adminClient.from('owner_editorial_calendar').upsert(row,{onConflict:'calendar_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'editorial_calendar_upserted','editorial_calendar',data.id,{title:data.title,event_date:data.event_date,auto_publish:data.auto_publish});res.status(201).json(data);}catch(error){res.status(400).json({error:error.message});}});
app.post('/api/admin/owner/business/calendar/:id/draft', admin, ownerWrite, async (req,res)=>{try{const {data,error}=await adminClient.from('owner_editorial_calendar').select('*').eq('id',req.params.id).single();if(error)throw error;if(!data)return res.status(404).json({error:'Calendar event tidak ditemukan'});const draft={title:data.title,summary:`Draft konten Muda untuk ${data.title}`,content_angle:data.content_angle,suggested_formats:data.suggested_formats,voice:data.brand_voice||'brand-muda',source:'owner_editorial_calendar'};const u=await adminClient.from('owner_editorial_calendar').update({status:'drafted'}).eq('id',data.id);if(u.error)throw u.error;await auditOwnerAction(req,'special_day_draft_generated','editorial_calendar',data.id,{draft});res.status(201).json({calendar:data,draft,auto_published:false});}catch(error){res.status(400).json({error:error.message});}});

app.get('/api/admin/owner/business/contacts', admin, async (_req,res)=>{try{const {data,error}=await adminClient.from('owner_audience_contacts').select('id,contact_key,display_name,email,phone,phone_verified,consent_status,source,purpose,retention_until,created_at,updated_at').order('updated_at',{ascending:false}).limit(500);if(error)throw error;res.json((data||[]).map(x=>({...x,phone:v19MaskPhone(x.phone)})));}catch(error){res.status(500).json({error:error.message});}});
app.post('/api/admin/owner/business/contacts', admin, ownerWrite, async (req,res)=>{try{const b=req.body||{};const row={contact_key:cleanText(b.contact_key,160)||`CONTACT-${Date.now()}`,display_name:cleanText(b.display_name,200)||null,email:cleanText(b.email,320)||null,phone:cleanText(b.phone,50)||null,phone_verified:b.phone_verified===true,consent_status:['unknown','granted','revoked'].includes(b.consent_status)?b.consent_status:'unknown',source:cleanText(b.source,80)||'owner',purpose:cleanText(b.purpose,200)||null,retention_until:b.retention_until||null,metadata:b.metadata&&typeof b.metadata==='object'?b.metadata:{}};const {data,error}=await adminClient.from('owner_audience_contacts').upsert(row,{onConflict:'contact_key'}).select().single();if(error)throw error;await auditOwnerAction(req,'audience_contact_upserted','audience_contact',data.id,{consent_status:data.consent_status,source:data.source});res.status(201).json({...data,phone:v19MaskPhone(data.phone)});}catch(error){res.status(400).json({error:error.message});}});
